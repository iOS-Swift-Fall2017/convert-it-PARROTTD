...
//  ConvertIT
//
//  Created by Duncan Parrott on 10/1/17.
...
class ViewController: UIViewController {
    ...
    let formulaArrays = [Formula(conversionString: "Miles to Kilometers", formula: {$0 / 0.62137}),
    ...
    override func viewDidLoad() {
       ...
       formulaPicker.dataSource = self
        ...
        conversionString = formulaArrays[formulaPicker.selectedRow(inComponent: 0)].conversionString
    ...
    func calculateConversion(){
        guard let inputValue = Double(userInput.text!) else{
            ...
            var outputValue = formulaArrays[formulaPicker.selectedRow(inComponent: 0)].formula(inputValue)
    ...
    @IBAction func signSegmentSelected(_ sender: UISegmentedControl) {
        ...
        }else { userInput.text = "-" + userInput.text!
...
extension ViewController: UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return formulaArrays.count
    }
    
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return formulaArrays[row].conversionString
    }
    
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        conversionString = formulaArrays[row].conversionString
        if conversionString.lowercased().contains("celcius".lowercased()){
            signSegment.isHidden = false
        } else { signSegment.isHidden = true
            userInput.text = userInput.text?.replacingOccurrences(of: "-", with: "")
            signSegment.selectedSegmentIndex = 0
            
        }
        let unitsArray = formulaArrays[row].conversionString.components(separatedBy: " to ")
